# -*- coding: utf-8 -*-
"""Production Bidirectional LSTM with SGD.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/15trHg3odaBzvtT-CcJrKgmGFx9zs56Gr
"""
...
# Define Bidirectional LSTM model with proper input shape
model = Sequential()
...
# Compile model with SGD optimizer and clipvalue to prevent exploding gradients
...
checkpoint_path = os.path.join(model_dir, "best_bilstm_model.h5")
...
try:
    history = model.fit(
        ...
        epochs=80,  # Increased epochs for SGD which may need more iterations to converge
    ...
    plt.savefig(os.path.join(model_dir, 'bilstm_training_history.png'))
    ...
    for i, target in enumerate(targets):
        ...
        plt.title(f'{target} - Actual vs Predicted (Bidirectional LSTM Model)')
        plt.legend()
        plt.savefig(os.path.join(model_dir, f'{target}_bilstm_predictions.png'))

    # Save model and scalers
    final_model_path = os.path.join(model_dir, "bilstm_production_forecasting.h5")
    model.save(final_model_path)

    # Save scalers
    with open(os.path.join(model_dir, "x_scaler_bilstm.pkl"), 'wb') as f:
        pickle.dump(X_scaler, f)

    with open(os.path.join(model_dir, "y_scaler_bilstm.pkl"), 'wb') as f:
        pickle.dump(y_scaler, f)

    print(f"Model saved to: {final_model_path}")
    print(f"Scalers saved to the models directory")
    print("Bidirectional LSTM model training and evaluation completed successfully!")

    # Compare with previous models if they exist
    gru_model_path = os.path.join(model_dir, "gru_production_forecasting.h5")
    lstm_model_path = os.path.join(model_dir, "lstm_production_forecasting.h5")

    print("\nComparison between Bidirectional LSTM and other models:")
    print("-----------------------------------------------------")
    print("Bidirectional LSTM advantages:")
    print("- Processes information in both forward and backward directions")
    print("- Often captures more complex patterns in sequential data")
    print("- Can learn dependencies regardless of their distance in the sequence")

    print("\nSGD optimization advantages:")
    print("- Often better generalization compared to adaptive optimizers in some cases")
    print("- Tends to find flatter minima which can result in more robust models")
    print("- Works well with momentum for escaping local minima")

    if os.path.exists(gru_model_path) or os.path.exists(lstm_model_path):
        print("\nYou can further compare performance metrics between models")
        print("by running all models and comparing the MAE, RMSE, and MAPE values")

except Exception as e:
    print(f"Error during Bidirectional LSTM model training or evaluation: {e}")
    import traceback
    traceback.print_exc()
...
def analyze_feature_importance():
    try:
        ...
        feature_model.add(Bidirectional(LSTM(32, input_shape=(seq_length, X_train.shape[2]))))
        feature_model.add(Dense(len(targets)))
        feature_model.compile(optimizer='sgd', loss='mse')
        ...
        plt.title('Relative Feature Importance for Production Prediction (Bidirectional LSTM)')
        plt.xlabel('Features')
        plt.ylabel('Relative Importance')
        plt.xticks(rotation=90)
        plt.tight_layout()
        plt.savefig(os.path.join(model_dir, 'bilstm_feature_importance.png'))
