...
# Define improved Bidirectional LSTM model
model = Sequential([
    Bidirectional(LSTM(128, return_sequences=True, dropout=0.2, recurrent_dropout=0.2), input_shape=(seq_length, len(features))),
    BatchNormalization(),
...
optimizer = tf.keras.optimizers.SGD(learning_rate=0.01, momentum=0.9)  # Added momentum for better convergence
model.compile(optimizer=optimizer, loss='mse', metrics=['mae'])

# Callbacks
early_stopping = EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True)
checkpoint_path = "./best_model_bilstm.h5"
...
future_df.to_csv("Future_3Months_Predictions_BiLSTM.csv", index=False)
print("Predictions saved! Check 'Future_3Months_Predictions_BiLSTM.csv' for results.")
...
plt.savefig("training_loss_bilstm.png")
...
for i, target in enumerate(targets):
    ...
    plt.legend()
    plt.savefig(f"{target}_forecast_bilstm.png")
