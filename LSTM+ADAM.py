...
Original file is located at
    https://colab.research.google.com/drive/1ahwh8S0ZF7c0EyO1s2-l3ZfFWAie7H2k
"""
...
# Load dataset
df = pd.read_excel("/content/202 GL.xlsx", sheet_name="Sheet1")
...
# Modified outlier detection with explicit bounds instead of Z-score
...
try:
    ...
    y_test_array = np.zeros((y_test.shape[0], len(X_columns) + len(y_columns)))
    y_test_array[:, -len(y_columns):] = y_test

    y_pred_array = np.zeros((y_pred.shape[0], len(X_columns) + len(y_columns)))
    y_pred_array[:, -len(y_columns):] = y_pred
    ...
    for i, target in enumerate(targets):
        ...
        plt.legend()
        plt.savefig(os.path.join(model_dir, f'{target}_predictions.png'))

    # Save model and scalers
    final_model_path = os.path.join(model_dir, "lstm_production_forecasting.h5")
